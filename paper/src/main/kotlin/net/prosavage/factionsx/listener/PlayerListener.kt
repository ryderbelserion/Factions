...
class PlayerListener : Listener {
    ...
    fun handleDeath(playerDeathEvent: PlayerDeathEvent) {
        ...
        if (factionAt.isWarzone() && Config.warZoneNoPowerLoss || factionAt.isSafezone() && Config.safeZoneNoPowerLoss
    ...
    fun handlePlayerDamage(event: EntityDamageByEntityEvent) {
        ...
        if (damagerEntity is Player || (damagerEntity is Projectile && damagerEntity.shooter is Player)) {
            ...
            if (damaged is Player) {
                ...
                //if (damagerFaction == damageeFaction && damager.hasFaction() && !damager.role.canDoPlayerAction(PlayerAction.HURT_PLAYER)) {
            ...
            val allowedToHitMob: Boolean = when {
                factionAt.isWarzone() -> ProtectionConfig.playerActionsInWarzone.getOrDefault(PlayerAction.HURT_MOB, true)

                factionAt.isWilderness() -> ProtectionConfig.playerActionsInWilderness[PlayerAction.HURT_MOB] ?: true

                factionAt.isSafezone() -> ProtectionConfig.playerActionsInSafezone.getOrDefault(PlayerAction.HURT_MOB, false)

                damagerFaction != factionAt && damager.hasFaction() -> factionAt.relationPerms.getPermForRelation(damagerFaction.getRelationTo(factionAt), PlayerAction.HURT_MOB)
    ...
    fun handleInteract(event: PlayerInteractEvent) {
        ...
        if (itemInHand != null && itemInHand != Material.AIR && allowMaterialInteractionGlobally[matchMaterial(itemInHand.name)]?.contains(matchMaterial(type.name)) == true) {
            return
        }
    ...
    fun PlayerCommandPreprocessEvent.onEnemyNearby() {
        ...
        if (Bukkit.getPluginCommand(command.substring(1)) == null || !Config.nearbyEnemyDisallowedCommands.any { command.equals("/$it", ignoreCase = true) } ||
    ...
    private fun processDamageAtGadget(factionAt: Faction, player: Player, entity: Entity, notify: Boolean): Boolean {
        ...
        if (notAllowed && notify) {
            actionbar(player, Message.youCannotDamageThisGadget, entity.type.name.toLowerCase(), factionAt.tag)
        }
    ...
    private fun processInteractAtEntity(factionAt: Faction, player: Player, entity: Entity, notify: Boolean): Boolean {
        ...
        if (notAllowed && notify) {
            actionbar(player, Message.youCannotInteractWithThisEntity, entity.type.name.toLowerCase(), factionAt.tag)
        }
    ...
    companion object {
        fun processBlockChangeInFactionLocation(
            ...
            if (action == BlockChangeAction.BREAK && factionPlayer.getFaction() == factionAt
                && ProtectionConfig.denyBreakingBlocksWhenEnemyNear.contains(changedMaterial.getMaterial()) && !factionPlayer.noEnemiesNearby(ProtectionConfig.denyBreakingBlocksEnemyNearByRadius)) {
                actionbar(player, Message.blockBreakDeniedEnemyNearby, changedMaterial.toString())
                return false
            }

            val allowed = when (action) {
                BlockChangeAction.PLACE -> handleActionProcessing(PlayerAction.PLACE_BLOCK, player, factionPlayer, factionAt, false, location)
                ...
                BlockChangeAction.EMPTY_BUCKET -> handleActionProcessing(PlayerAction.EMPTY_BUCKET, player, factionPlayer, factionAt, false, location)

                BlockChangeAction.FILL_BUCKET -> handleActionProcessing(PlayerAction.FILL_BUCKET, player, factionPlayer, factionAt, false, location)
            }
        ...
        fun processActionInFactionsLand(
            ...
            if (action == Action.PHYSICAL) {
                val exactMaterial = clickedBlock.type.getMaterial()
                if (exactMaterial == Material.TRIPWIRE
                    && !handleActionProcessing(PlayerAction.HOOK, player, fPlayer, factionAt, true, location)) {
                    return false
                }

                if (exactMaterial?.isPressurePlate() == true
                ...
                if (exactMaterial === Material.FARMLAND && !handleActionProcessing(PlayerAction.TRAMPLE_SOIL, player, fPlayer, factionAt, true, location)) {
                    return false
                }
            }
            ...
            if (action == Action.RIGHT_CLICK_BLOCK) {
                ...
                val playerAction: PlayerAction? = when (matchMaterial(clickedBlock.type.name)) {
