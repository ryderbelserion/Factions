...
class FactionRoles(var roleHierarchy: HashMap<Int, CustomRole>) {
    ...
    fun replaceRoles(role: CustomRole, roleToReplace: CustomRole) {
        val indexOfFirst = roleHierarchy.entries.find { it.value == role }!!
        val indexOfReplaced = roleHierarchy.entries.find { it.value == roleToReplace }!!
    ...
    fun isHigherRole(first: CustomRole, second: CustomRole): Boolean {
        val firstRole = roleHierarchy.filterValues { customRole -> customRole == first }.keys.firstOrNull() ?: return false
        val fplayerRole = roleHierarchy.filterValues { customRole -> customRole == second }.keys.firstOrNull() ?: return false
    ...
    fun getPromotionRole(recipient: FPlayer): CustomRole? {
        ...
        //val currentRoleIndex = roleHierarchy.filterValues { customRole -> customRole == recipient.role }.keys.firstOrNull() ?: return recipient.role
