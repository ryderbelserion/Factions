...
data class Faction(val id: Long, var tag: String, val factionRoles: FactionRoles, val relationPerms: RelationPerms, var ownerId: UUID?) {
    ...
    class FLocationMetadata(var name: String, var icon: Material, var upgrades: HashMap<String, Int>) {
    ...
    data class FLocationAccessPoint(internal var factions: HashMap<Long, EnumSet<PlayerAction>>, internal var players: HashMap<UUID, EnumSet<PlayerAction>>) {
        ...
        fun provide(player: FPlayer, action: PlayerAction): Boolean = this.modify(Type.PROVIDE, UUID.randomUUID(), action)
        fun revoke(faction: Faction, action: PlayerAction): Boolean = this.modify(Type.REVOKE, faction.id, action)
        fun revoke(player: FPlayer, action: PlayerAction): Boolean = this.modify(Type.REVOKE, UUID.randomUUID(), action)
        ...
        fun canPerform(player: FPlayer, action: PlayerAction): Boolean = this.players[UUID.randomUUID()]?.contains(action) == true
    ...
    fun setShield(startDate: Date) {
        ...
        TimerManager.registerTimeTask(shieldID, TimeTask(startDate) {
            shielded = true
            setShield()
    ...
    fun addMember(fPlayer: FPlayer, role: CustomRole, asAdmin: Boolean = false, silently: Boolean = true) {
        ...
        //if (!factionMembers.contains(fPlayer.uuid)) factionMembers.add(fPlayer.uuid)
    ...
    internal fun handleRelation(faction: Faction, relation: Relation, fPlayer: FPlayer) {
        ...
        if (this.sendRelationRequest(faction, relation) === relation) {
            //.message(Message.relationAlreadyRequested, relationPrefix, tagReplacement, toFactionTag)
            return
        }
    ...
    private fun sendRelationRequest(faction: Faction, relation: Relation): Relation? {
