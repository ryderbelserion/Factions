...
class CmdTruce : FCommand() {
    init {
        ...
        commandRequirements = CommandRequirementsBuilder()
                .withPermission(Permission.TRUCE)
                .withMemberAction(MemberAction.RELATION)
                .withPrice(EconConfig.fTruceCost)
                .build()
    }
    ...
    override fun execute(info: CommandInfo): Boolean {
        ...
        if (trigFaction.getRelationTo(faction) == Relation.TRUCE) {
            info.message(Message.relationAlreadySet, Relation.TRUCE.tagReplacement, faction.tag)
            return false
        }

        if (trigFaction.getRelationalFactions(Relation.TRUCE).size >= trigFaction.getMaxTruces()) {
            info.message(Message.relationMax, Relation.TRUCE.tagReplacement)
            return false
        }

        //trigFaction.handleRelation(faction, Relation.TRUCE, info.fPlayer ?: return false)*/
        return true
    }

    override fun getHelpInfo(): String {
        return Message.commandTruceHelp
    }
}