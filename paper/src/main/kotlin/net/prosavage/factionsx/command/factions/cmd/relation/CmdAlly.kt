...
class CmdAlly : FCommand() {
    init {
        ...
        commandRequirements = CommandRequirementsBuilder()
                .withPermission(Permission.ALLY)
                .withMemberAction(MemberAction.RELATION)
                .withPrice(EconConfig.fAllyCost)
                .build()
    }
    ...
    override fun execute(info: CommandInfo): Boolean {
        ...
        if (trigFaction.getRelationTo(faction) == Relation.ALLY) {
            info.message(Message.relationAlreadySet, Relation.ALLY.tagReplacement, faction.tag)
            return false
        }

        if (trigFaction.getRelationalFactions(Relation.ALLY).size >= trigFaction.getMaxAllies()) {
            info.message(Message.relationMax, Relation.ALLY.tagReplacement)
            return false
        }

        //trigFaction.handleRelation(faction, Relation.ALLY, info.fPlayer ?: return false)*/
        return true
    }

    override fun getHelpInfo(): String {
        return Message.commandAllyHelp
    }
}