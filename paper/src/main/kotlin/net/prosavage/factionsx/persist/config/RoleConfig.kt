...
object RoleConfig : IConfigFile {
            ...
            FactionRoles(
                    hashMapOf(
                            ...
                            4 to CustomRole(
                                    "***",
                                    "Leader",
                                    PlayerAction.entries.filter { action -> action != PlayerAction.HURT_PLAYER }.toMutableList(),
                                    MemberAction.entries.toMutableList(),
                                    HashMap(),
                            ...
                            3 to CustomRole(
                                    ...
                                    "Administrator",
                                    PlayerAction.entries.filter { action -> action != PlayerAction.HURT_PLAYER }.toMutableList(),
                                    MemberAction.entries.toMutableList().filter { action -> action != MemberAction.DISBAND }
                                            .toMutableList(),
                                    HashMap(),
    ...
    override fun load(factionsx: FactionsX) {
        //Serializer(false, factionsx.dataFolder, factionsx.logger)
         //       .load(RoleConfig.instance, RoleConfig::class.java, File("${factionsx.dataFolder}/config", "role-config.json"))
    }
}