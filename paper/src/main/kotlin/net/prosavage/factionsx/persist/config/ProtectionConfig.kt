...
object ProtectionConfig : IConfigFile {
    ...
    var allowMaterialInteractionGlobally = mapOf(
            Material.ENDER_PEARL to Material.entries.filter { it.name.endsWith("DOOR") }.toSet()
    )
    ...
    var shiftRightClickableWhiteListInOtherFactionsLand = hashMapOf(
            Material.CREEPER_SPAWN_EGG.getMaterial() to listOf(
                    Material.CHEST.getMaterial(),
                    Material.TRAPPED_CHEST.getMaterial(),
                    Material.ENDER_CHEST.getMaterial()
            )
    )
    ...
    fun populateOptions(instance: FactionsX) {
        PlayerAction.entries.forEach { action ->
            ...
            Relation.entries.forEach { rel ->
                ...
                if (defaultActionsInOtherFactionsLand.info == null) defaultActionsInOtherFactionsLand.info = hashMapOf()
                defaultActionsInOtherFactionsLand.info.putIfAbsent(rel, HashMap())
        ...
        EntityType.entries.forEach { entityType ->
            // smol mod support
            if (entityType.name !in allowedEntities) return@forEach

            if (entityType != EntityType.PLAYER) allowedInteractableEntitiesInOtherFactionLand.putIfAbsent(entityType, false)
