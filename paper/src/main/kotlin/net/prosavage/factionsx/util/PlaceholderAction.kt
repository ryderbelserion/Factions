...
enum class PlaceholderAction(val holder: String?, val chars: Int) {
    ...
    companion object {
        ...
        private fun Collection<List<String>>.valuePercentageLastList(): List<String>? = lastOrNull { value -> value.any { it.matches(Patterns.VALUE_HOLDER_PERCENTAGE) } }

        private fun List<String>?.valuePercentageCollect(): Int? = this?.lastOrNull { it.matches(Patterns.VALUE_HOLDER_PERCENTAGE) }?.substring(16)?.toIntOrNull()
    }
}