...
fun Material.getMaterial(): Material? {
...
internal fun asyncIf(predicate: Boolean, action: () -> Unit) = if (predicate) commonPool().execute(action) else action()
...
fun Faction.isActionsWhenOfflineCompatible(actionsWhenOffline: ProtectionConfig.ActionsWhenOffline): Boolean {
    return !this.isSystemFaction()
            ...
            //&& System.currentTimeMillis() >= this.getMembers().maxBy { it.timeAtLastLogin }.timeAtLastLogin.plus(actionsWhenOffline.hasBeenOfflineForSeconds * 1000)
}
